...
def retrain_bo3_model(master):
    ...
    del serve
    gc.collect()
    return model, maeimport pandas as pd
import numpy as np
import re
import streamlit as st
from sklearn.ensemble import RandomForestClassifier, RandomForestRegressor
...
# ──────────────────────────────────────────────────────────────────────────────
# 3. СТАТИСТИКИ НА ИГРАЧИТЕ (кешира се)
# ──────────────────────────────────────────────────────────────────────────────
@st.cache_data(ttl=21600, show_spinner="⚙️ Изчислява статистики...")
def build_player_stats(_master):
    master = _master
    ...
    # Възраст
    ages = pd.concat([
        master[['tourney_date','winner_name','winner_age']].rename(columns={'winner_name':'player','winner_age':'age'}),
        master[['tourney_date','loser_name','loser_age']].rename(columns={'loser_name':'player','loser_age':'age'})
    ]).sort_values(['player','tourney_date'])
    latest_age = ages.groupby('player')[['age']].last()
    ...
    # Ранглиста
    ranks = pd.concat([
        master[['tourney_date','winner_name','winner_rank']].rename(columns={'winner_name':'player','winner_rank':'rank'}),
        master[['tourney_date','loser_name','loser_rank']].rename(columns={'loser_name':'player','loser_rank':'rank'})
    ]).sort_values(['player','tourney_date'])
    latest_rank = ranks.groupby('player')[['rank']].last()
...
def build_features(p1, p2, surface, stats, master):
    ...
    r1 = float(s1.get('rank', 200) or 200)
    r2 = float(s2.get('rank', 200) or 200)
    # За BO3: "winner" е играчът с по-добра ранглиста (по-нисък номер)
    top = s1 if r1 <= r2 else s2
    bot = s2 if r1 <= r2 else s1
...
# ──────────────────────────────────────────────────────────────────────────────
# 6. BACKTEST — тества модела на последните N месеца
# ──────────────────────────────────────────────────────────────────────────────
def backtest_models(winner_models, bo3_model, master, stats, months=6):
    """
    Тества моделите на последните `months` месеца данни.
...
# ──────────────────────────────────────────────────────────────────────────────
# 7. RETRAIN — преобучава всички модели с най-нови данни
# ──────────────────────────────────────────────────────────────────────────────
def retrain_winner_model(original_model, surface, master, stats, train_until_year=2025):
    """Преобучава winner модел с разширен dataset."""
    feat_list = SURFACE_FEATURES[surface]
    df = master[
        (master['surface'] == surface) &
        (master['tourney_date'].dt.year <= train_until_year)
    ...
    for _, row in df.iterrows():
        try:
            # Положителен пример: winner wins
            feats_pos = {
                'elo_surf_diff': float((stats.loc[row['winner_name']].get(f'elo_{surface}', 1500) or 1500)
                                      - (stats.loc[row['loser_name']].get(f'elo_{surface}', 1500) or 1500))
                if row['winner_name'] in stats.index and row['loser_name'] in stats.index else 0,
                '1st_won_diff': float((stats.loc[row['winner_name']].get('roll_first', 0.65) or 0.65)
                                     - (stats.loc[row['loser_name']].get('roll_first', 0.65) or 0.65))
                if row['winner_name'] in stats.index and row['loser_name'] in stats.index else 0,
                'age_diff': float((stats.loc[row['winner_name']].get('age', 26) or 26)
                                  - (stats.loc[row['loser_name']].get('age', 26) or 26))
                if row['winner_name'] in stats.index and row['loser_name'] in stats.index else 0,
                'form_diff': float((stats.loc[row['winner_name']].get('roll_form', 0.5) or 0.5)
                                   - (stats.loc[row['loser_name']].get('roll_form', 0.5) or 0.5))
                if row['winner_name'] in stats.index and row['loser_name'] in stats.index else 0,
                'rest_days_diff': 0,
                'h2h_cum': 0,
                'h2h_surf_roll': 0.5,
            ...
            feat_row = [feats_pos.get(f, 0) for f in feat_list]
            rows.append(feat_row + [1])  # label = 1 (winner wins)

            # Отрицателен пример: loser side (огледален)
            rows.append([-v for v in feat_row] + [0])
        except Exception:
            pass
...
def retrain_bo3_model(master):
    """Преобучава BO3 модела с всички налични данни (включва 2026)."""
    import gc
    feat_list = BO3_FEATURES
    ...
    if not test.empty:
        from sklearn.metrics import mean_absolute_error
        preds = model.predict(test[feat_list])
        mae = mean_absolute_error(test['total_games'], preds)
